...
char *float2_to_char1(float **float_2array, int size, int dimention){
	char * char_1array = new char[size*dimention*sizeof(float)];
	char *p = char_1array;


	for (int i = 0, j = 0; i < size*dimention*sizeof(float); i= i + dimention*sizeof(float)){

		memcpy(&char_1array[i],float_to_char(float_2array[j], dimention),dimention*sizeof(float) );
		delete [] float_2array[j];
		j++;
...
float **char1_to_float2(char * char_1array, int size, int dimention){
	float **float_2array; 
	float_2array = new float *[size];
	for (int i = 0,  j = 0; i < size*dimention*sizeof(float); i= i + dimention*sizeof(float)){
	    float_2array[j] = char_to_float(&char_1array[i], dimention);
		j++;
	}
	delete [] char_1array;
	return float_2array;
...
void DATA::transfer_float2_to_char1()		
{ 
	char1 = float2_to_char1(emb_data,m_size ,m_dimention);
}
void DATA::transfer_char1_to_float2()		
{ 
	emb_data = char1_to_float2(char1,m_size , m_dimention);
}